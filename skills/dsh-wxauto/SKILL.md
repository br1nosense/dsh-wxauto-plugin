...
- `sender_whitelist`：**群聊发送者白名单（P0 安全）**（数组）。群聊里只有白名单内的成员能驱动 agent
  （发 `/命令`、直接发普通消息当任务、回答 DSH 提问）。
  - **空（默认）= 群聊默认拒绝所有成员驱动**（安全方向：防止群里任何人 @ 你就能操控 agent）
  - `["*"]` = 显式开放：不做 sender 鉴权，所有 @ 都接受（兼容旧行为）
  - `["张三","李四"]` = 只允许名单内的人驱动；私聊不受此限制（聊天窗口唯一确定对方）
...
| `senderWhitelist` | 群聊发送者白名单（空=默认拒绝；* = 开放） | `sender_whitelist` |
...
/cancel          取消当前会话回合（并清理其待推送/等待状态）
/stop            终止当前会话任务（取消回合 + 清理待推送，防卡死）
/stopall         终止本聊天所有卡死任务（含已归档会话，清空全部待推送）
```
...
- **群聊策略（白名单 + 发送者鉴权 + @ 响应）**：白名单（`group_whitelist`）内的群**会自动加入轮询/监听**（无需再写进
  `bridgeChats`/`listenChats`）；开启 `group_mention_only`（默认）时，群聊里只有 **@ 你** 的消息才会被受理
  （命令/自动回复/任务），避免控制群一活跃就刷屏。群聊发「@机器人 /list」即可命令（桥自动剥掉开头 @ 前缀）。
  识别不到你的昵称时会告警（可在设置 `myAliases` 配置群名片等昵称别名）。
  ⚠️ **发送者鉴权（P0 安全）**：群聊里驱动 agent（发命令/任务/回答提问）的成员必须在 `sender_whitelist` 内 ——
  **未配置时群聊默认拒绝所有成员驱动**（防止群里任何人 @ 你就能操控你的 agent）；配置 `["*"]` 才显式开放
  （不做 sender 鉴权，所有 @ 都接受）。私聊不受此限制（聊天窗口唯一确定对方）。
  ⚠️ 首次把某个群加入白名单后，群里**已有的可见消息**会被当作新消息受理一遍（含 @ 你的历史消息），
  建议用新群测试或先清空。
- **防重复回复（P0 修复）**：wxauto4 的 `SendMsg` 偶发「实际已发送但返回状态不是成功」，旧实现会重试**再发一条**，
  导致同一条回复在微信里出现多条。现已在发送层做**短窗口去重**（同一聊天 1.5s 内相同文本不重发），
  且 `SendMsg` 调用一次即视为已发送（不再因响应误判而重复发送）；提问转发也按 `rpc_id` 去重，避免同一提问推多次。
...
- **忙会话自动新建（防止「已受理但 DSH 不执行」）**：提交任务前检查 active 会话是否正在运行。若 active 会话忙碌（例如 agent 卡在等用户回答的 `ask_user_question`、或正处理长任务），任务**不排队**到该会话，而是**自动新建会话执行** —— 否则排队任务会一直不跑（表现为微信回了「已受理任务」但 DSH 里毫无进展）。空闲会话则复用（保持上下文）。
- **归档/中断自动清理（防止「卡死在此任务里」）**：手动归档对话、删除会话或中途中断后，会话在 DSH 侧不再存在。桥会自动检测：提交任务前、等待完成前、重启补发前都会检查会话是否还存在 —— 已归档的会话**自动清理**其待推送/等待状态，不再傻等回合结束（否则 `wait_turn_end` 会一直等到超时，桥卡死在该任务上，且重启后反复补发）。配合手动指令：
  - `/cancel`：取消当前会话回合 + 清理其待推送/等待
  - `/stop`：终止当前会话任务（同上，语义更明确）
  - `/stopall`：终止本聊天**所有**卡死任务（含已归档会话），清空全部待推送 —— 手动归档后用它一键清理
  - `/new`：新建对话并自动清理旧会话卡死残留
- **DSH 提问转发（ask_user_question）**：agent 需要用户选择/回答时，桥订阅 DSH `events.mux`（需 `pip install websocket-client`），把问题+选项**推送微信**（编号列出）。用户在微信回复「选项编号/选项内容」（多问题用「序号: 答案」分行；回复 0 跳过；多选用逗号分隔）即回填给 agent，任务继续。等待回答超时（默认 600s，配置 `question_timeout`）自动跳过并通知。等待期间桥不会误判回合结束/超时。
